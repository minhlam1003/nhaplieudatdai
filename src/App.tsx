...
export default function App() {
  ...
  const filteredData = useMemo(() => {
    let result = data.filter(item => {
      ...
      return (
        item.hoTenChuSuDung?.toLowerCase().includes(query) ||
        item.maHoSoLuuTru?.toLowerCase().includes(query) ||
        item.tieuDeHoSo?.toLowerCase().includes(query) ||
        item.soGiayChungNhan?.toLowerCase().includes(query)
      );
    });
  ...
  const handleBatchDelete = async () => {
    ...
    if (confirm(`Bạn có chắc chắn muốn xóa ${selectedIds.size} hồ sơ đã chọn?`)) {
      try {
        const promises = Array.from(selectedIds).map(id => deleteDoc(doc(db, 'records', String(id))));
        await Promise.all(promises);
        setSelectedIds(new Set());
        addNotification('success', `Đã xóa thành công ${selectedIds.size} hồ sơ!`);
      } catch (error) {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-50/80 backdrop-blur-md">
          ...
          >
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      type="password" 
                      className="w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl text-sm focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium"
                      placeholder="Password (mặc định: admin)"
            ...
            <footer className="p-6 bg-gray-50 border-t border-gray-100 text-center">
              <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Trung tâm lưu trữ địa chính v2.4.0</p>
            </footer>
          </motion.div>
        </div>
      )}
      ...
      <div className="flex flex-col h-screen overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-8 flex-shrink-0 z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => activeTab === 'entry' && handleSaveEntry()}
              className={cn(
                "px-6 py-2 rounded-lg text-sm font-semibold shadow-sm transition-all",
                activeTab === 'entry' 
                  ? "bg-blue-600 text-white hover:bg-blue-700 shadow-blue-100" 
                  : "bg-gray-100 text-gray-400 cursor-not-allowed"
              )}
              disabled={activeTab !== 'entry'}
            >
              Lưu Dữ Liệu
            </button>
          </div>
          ...
          <aside className="w-72 bg-white border-r border-gray-200 flex flex-col p-6 space-y-6 overflow-y-auto">
            ...
            <div className="mt-auto space-y-3">
                ...
                <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <p className="text-[11px] text-gray-600 font-medium">Khởi tạo lúc: {new Date().toLocaleTimeString()}</p>
                </div>
            </div>
          </aside>
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col justify-between">
                      <div>
                        ...
                        <button 
                          ...
                          className="text-sm font-bold text-gray-800 hover:text-blue-600 flex items-center gap-2 transition-colors"
                        >
                        ...
                        <tbody className="divide-y divide-gray-50 text-xs">
                          {allUsers.map((user) => (
                            <tr key={user.uid} className="hover:bg-gray-50/50 transition-colors">
                              <td className="px-6 py-4 font-medium text-gray-900">{user.username || user.email}</td>
                      ...
                      <div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                          <div className="relative">
                            ...
                            <input
                              type="text"
                              placeholder="Nhập tên chủ, GCN hoặc mã mã hồ sơ..."
                              className="bg-white border border-gray-200 rounded-xl pl-11 pr-6 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all w-full md:w-80 font-medium"
                              value={searchQuery}
                            ...
                            {filteredData.length > 0 ? (
                              ...
                              <tr>
                                <td colSpan={8} className="p-16 text-center">
                                  ...
                                  <p className="text-sm text-gray-400 italic font-medium">Không tìm thấy kết quả phù hợp với từ khóa</p>
                                </td>
                              </tr>
                            )}
                          </tbody>
